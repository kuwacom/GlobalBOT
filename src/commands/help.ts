...
import ButtonFormat, * as FormatButton from "../format/button";
import env from "../config/env";
import { embedConfig } from "../config/discord";
import { DiscordCommandInteraction } from "../types/discord";
import { slashCommands } from "../utiles/discord";
...
export const executeMessage = async (message: Discord.Message) => {
    ...
    slashCommands.forEach((command) => {
        if (command.name in env.commands) {
            fields.push({
                name: `${env.prefix}${command.name}`,
